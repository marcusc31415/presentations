...
class DemoScene3(PresentationScene):
    def construct(self):
            ...
            self.play(mn.Rotate(tree_1_lines[1], angle=np.pi, axis=np.array([0, 1, 0]), about_point=tree_1_lines[1].start), mn.Rotate(tree_1_lines[2], angle=np.pi, axis=np.array([0, 1, 0]), about_point=tree_1_lines[2].start))
            self.end_fragment()
