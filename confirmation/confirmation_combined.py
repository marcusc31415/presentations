...
class Confirmation(PresentationScene):
    def construct(self):
        ...
        self.play(timeline.animate.shift(10*mn.DOWN))
        ...
        self.remove(*group_2)
        self.remove(rect)
        self.play(timeline.animate.shift(10*mn.UP))
        self.end_fragment()
        ...
        text1, _, _ = create_paragraph(r"There are three ways an automorphism can act on a tree: ", "fix a vertex", ",\\ ", "invert an edge", ", or ", "translate along an axis", ".")
        ...
        for i, row in enumerate(blist):
            ...
            if i == 0:
                ...
                self.end_fragment()
                self.remove(*fv)
                text1[2].set_color(mn.WHITE)
                text1[4].set_color(mn.YELLOW)
                self.end_fragment()
                self.add(ie)
                self.end_fragment()
                self.play(mn.Rotate(ie, angle=np.pi, about_point=mn.ORIGIN, axis=mn.UP))
                self.end_fragment()
                text1[4].set_color(mn.WHITE)
                text1[6].set_color(mn.YELLOW)
                self.remove(*ie)
                self.end_fragment()
                self.add(trans)
                self.end_fragment()
